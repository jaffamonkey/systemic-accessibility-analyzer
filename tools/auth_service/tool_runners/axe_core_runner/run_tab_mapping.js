...
export async function generateFullTabMap() {
    ...
    const focusable = rawElements
        ...
        // 3. Render clean page to Canvas
        const cleanCanvas = await html2canvas(document.body, {
        allowTaint: true,
        useCORS: true,
        logging: false,
        ...
        cleanLink.download = `visual-preview-${Date.now()}.png`;
        cleanLink.href = cleanCanvas.toDataURL('image/png');
        cleanLink.click();

        // Clone canvas for tab map overlay
        ...
        // 4. Draw overlays onto cloned canvas
        ctx.lineWidth = 2;
    ...
    link.download = `tab-map-${Date.now()}.png`;
