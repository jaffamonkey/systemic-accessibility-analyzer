...
def _field_text(page: Page, field: Locator) -> str:
    """
    Build a useful text blob for a field:
    - id/name/type/autocomplete/placeholder/aria-label/data-testid
...
def _score_username(page: Page, field: Locator) -> int:
    ...
    if USERNAME_WORDS.search(text):
        score += 70

    if BAD_USERNAME_WORDS.search(text):
        score -= 120
...
def _score_submit(button: Locator) -> int:
    ...
    if SUBMIT_WORDS.search(text):
        score += 80

    if BAD_SUBMIT_WORDS.search(text):
        score -= 120
...
def _nearest_form_or_region(page: Page, field: Locator) -> Locator:
    """
    Prefer the actual form. If no form exists, use a nearby dialog/main/container.
    This helps with React login widgets that do not use real <form>.
    """
    try:
...
def detect_login_form(page: Page):
    ...
    - password field anchors the candidate form
    - username is chosen from the same form/region
    - submit is chosen from the same form/region
    """
    ...
    for selector in selectors:
        try:
            ...
            for i in range(min(locators.count(), 40)):
                ...
                if LOGIN_WORDS.search(combined):
                    score += 100

                if re.search(r"/(login|signin|sign-in|account|auth)\b", href, re.I):
                    score += 70

                if re.search(r"\b(register|sign up|basket|cart|search|menu)\b", combined, re.I):
                    score -= 60
