...
# Run from the tool_runners folder, or pass that folder as the first argument.
ROOT_DIR="${1:-$(pwd)}"
...
cleanup_outputs() {
  ...
  local -a tool_dirs=(
    ...
    "$ROOT_DIR/alfa"
    "$ROOT_DIR/aslint"
    "$ROOT_DIR/editoria11y"
    "$ROOT_DIR/nu-html-checker"
    "$ROOT_DIR/qualweb"
...
run_direct_node_runner() {
  local tool_name="$1"
  local script_name="$2"
  local dir="$ROOT_DIR/$tool_name"

  cd "$dir"
  npm install
  npx playwright install chromium

  # These newer runners follow the auth-service/job-dir shape:
  #   node run_x.js <job_dir>
  #
  # For this all-tools orchestrator, the tool folder itself acts as the job dir.
  # Expected input:
  #   <tool-folder>/input/urls.txt
  #
  # Expected output:
  #   <tool-folder>/reports/<tool-name>/*.json
  node "$script_name" "$dir"
}

run_alfa() {
  run_direct_node_runner "alfa" "run_alfa.js"
}

run_aslint() {
  run_direct_node_runner "aslint" "run_aslint.js"
}

run_editoria11y() {
  run_direct_node_runner "editoria11y" "run_editoria11y.js"
}

run_nu_html_checker() {
  run_direct_node_runner "nu-html-checker" "run_nu_html_checker.js"
}

run_qualweb() {
  run_direct_node_runner "qualweb" "run_qualweb.js"
}
...
copy_reports_to_collection() {
  ...
  copy_first_existing_dir "alfa" \
    "$ROOT_DIR/alfa/reports/alfa" \
    "$ROOT_DIR/alfa/reports"

  copy_first_existing_dir "aslint" \
    "$ROOT_DIR/aslint/reports/aslint" \
    "$ROOT_DIR/aslint/reports"

  copy_first_existing_dir "editoria11y" \
    "$ROOT_DIR/editoria11y/reports/editoria11y" \
    "$ROOT_DIR/editoria11y/reports"

  copy_first_existing_dir "nu-html-checker" \
    "$ROOT_DIR/nu-html-checker/reports/nu-html-checker" \
    "$ROOT_DIR/nu-html-checker/reports"

  copy_first_existing_dir "qualweb" \
    "$ROOT_DIR/qualweb/reports/qualweb" \
    "$ROOT_DIR/qualweb/reports"
...
main() {
  ...
  run_step alfa run_alfa
  run_step aslint run_aslint
  run_step editoria11y run_editoria11y
  run_step nu-html-checker run_nu_html_checker
  run_step qualweb run_qualweb
